...
**第 4 ラウンド(非 CJK・機能/描画のエッジケース 14 図を新規作成、Opus で目視)** — 検出・修正:
state の**3 並行領域が逐次チェーン化**(各領域の初期 `[*]` が 1 個に共有され領域をまたぐ →
pseudoId を領域単位に。第 1 ラウンドの「終端 `[*]` 共有」修正が並行領域で行き過ぎていた回帰)/
sequence の **create/destroy が未描画**(生成参加者が最初から存在し destroy が無視 →
生成位置からライフライン開始 + destroy 位置に ✕・フッタ省略)。
**データバグでない描画アーティファクトと判定して保留**: dense flowchart のエッジ重なり/ノード貫通、
er の多対多で 2 エッジが同じ辺に寄りカーディナリティ記号が重なる——いずれも出力 XML は正しく、
**draw.io ビューアで libavoid(障害物回避 wasm)が CSP で無効化**されているのが主因(実アプリでは改善)。

**第 3 ラウンド(機械的ブロードスイープ、71 変換ケース + 22 警告ケース)** — 目視ではなく
プログラムで全図式 × 多構文を一気通しし、**空図・クラッシュ・不正 XML・有効入力での警告**を検出:
requirement パーサも CJK 識別子で全滅(6 番目のパーサ)/ state の単独状態宣言
(`待機` を単独行)+ note が未対応で空図 / flowchart の `style`/`class` ディレクティブが
CJK ノードを対象にできずスタイル脱落。この 3 種を修正。**このスイープは
`test/broad-sweep.test.js` として恒久テスト化済み**(CJK バグを最初に捕まえた網)。

### 機械スイープの勘所(目視 QA を補完する)
- **空図の署名 = `<mxCell>` が 2 個以下**(ルートのみ)。これを全 kind × 多構文で検査すると、
  「テストは緑・レンダラは無言」の CJK 系バグが一撃で出る。
- **有効入力に警告が 1 つでも出たら要調査**(= パース漏れ)。ただし意図的に未対応の構文
  (`click`、一部 beta 構文)は警告が正しいので、警告ケースは「完全対応のはずの入力」に絞る。
- 特に **CJK を二次的な位置**(`style <ノード>`、`Note over <参加者>`、カーディナリティ両端、
  関係ラベル)に散らす。一次位置(ノード宣言)だけ直して二次位置を忘れがち。
