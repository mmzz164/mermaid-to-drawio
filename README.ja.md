...
これで標準的な Mermaid 図式はすべてネイティブ変換できます。オプションの png/svg 画像埋め込みモードは、今後追加される新図式などの未対応分に対するフォールバックとして残しています。対応図式内の未対応構文は警告を出してスキップし、変換は継続します(graceful degradation)。日本語などの CJK は**ラベルだけでなく識別子**でも全図式で使えます。
...
png/svg 画像埋め込みモード(未対応の図式に対するフォールバック)を使う場合はオプション依存を追加します:
...
| `tool/test/` | テスト約 290 件。全図式の出力をバイト単位で固定するゴールデンスナップショットを含む |
